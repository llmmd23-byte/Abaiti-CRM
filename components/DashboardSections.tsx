...
export function RentalContractsPanel({locale}: {locale: string}) {
  ...
  const [saveStatus, setSaveStatus] = useState("");
  const [payingContractIds, setPayingContractIds] = useState<Set<number>>(() => new Set());
  const [editingContractId, setEditingContractId] = useState<number | null>(null);
  ...
  const visibleBoothMapLayout = useMemo(() => {
    ...
    return PPT_BOOTH_LAYOUT.filter((booth) => {
      if (!boothCatalogByNumber.has(booth.id.toUpperCase())) return false;
  ...
  function selectBooth(nextBooth: string) {
    ...
    const booth = boothGroups
      .flatMap((group) => group.booths)
      .find((item) => item.id.toUpperCase() === normalizedBooth);
  ...
  function editContract(contract: BackendRow) {
    ...
    rentalAutoSaveSnapshotRef.current = JSON.stringify({
      ...
      booth_number: String(contract.booth_number ?? "").trim() || null,
    ...
    setBoothNumber(contractText(contract.booth_number, ""));
  ...
  async function markContractPaymentPaid(contract: BackendRow) {
    const contractId = Number(contract.id);
    if (!Number.isFinite(contractId) || payingContractIds.has(contractId)) return;
    setPayingContractIds((current) => new Set(current).add(contractId));
    setSaveStatus(text.saving);
    try {
      await updateBackend("rental-contracts", contractId, {payment_status: "paid"});
      await contracts.reload();
      await rentalBooths.reload();
      setSaveStatus(text.updated);
    } catch {
    ...
    <div className="quotes-page-grid">
      ...
      <article className="quote-card quote-history-card">
        ...
        <div className="quote-history-table"><table><thead><tr><th>{isArabic ? decodeEscapedText("\u0631\u0642\u0645 \u0627\u0644\u0639\u0642\u062f") : "Contract #"}</th><th>{text.customer}</th><th>{text.companyName}</th><th>{text.rentalItem}</th><th>{text.paymentStatus}</th><th>{text.contractDate}</th><th>{text.actions}</th></tr></thead><tbody>
          {filteredContracts.map((contract) => {
            const paymentValue = String(contract.payment_status ?? "pending_payment").toLowerCase();
            const paymentLabel =
              paymentValue === "paid"
                ? text.paid
                : text.pendingPayment;
            const isPaying = payingContractIds.has(Number(contract.id));
            return (
              <tr key={contract.id}>
                ...
                <td>
                  <div className="rental-payment-cell">
                    {paymentValue === "paid" ? (
                      ...
                      <button
                        className="quote-status pending_payment rental-payment-mark-paid"
                        disabled={isPaying}
                        onClick={() => void markContractPaymentPaid(contract)}
                        type="button"
                      >
                        {isPaying ? text.saving : text.paid}
                      </button>
                    )}
                  </div>
                </td>
