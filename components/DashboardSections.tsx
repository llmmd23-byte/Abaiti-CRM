...
function appliesToContract(settings: ContractSettings | null, type: "sponsorship" | "participation" | "rental") {
...
function printAreaSevenRentalContractThreePage(contract: BackendRow) {
  const printWindow = window.open("", "_blank", "width=900,height=1100");
  if (!printWindow) {
    window.alert("يرجى السماح بالنوافذ المنبثقة لفتح نافذة الطباعة.");
    return;
  }
  const get = (field: string, fallback = "") => String(contract[field] ?? fallback).trim() || fallback;
  ...
  const amount = Number(contract.subtotal ?? (Number(contract.unit_price ?? 0) * Number(contract.quantity ?? 1))) || 0;
  const vat = Number(contract.vat_amount ?? amount * 0.15) || amount * 0.15;
  const total = Number(contract.grand_total ?? amount + vat) || amount + vat;
  ...
  const header = `<div class="header-banner"><div class="header-title-box">عقد تأجير مساحة جناح (بوث)<br>في معرض ملتقى أمن وسلامة الفعاليات الثقافية والفنية بجدة</div><div class="header-logo-box"><strong>AREA SEVEN</strong>رقم العقد: ${contractNumber}<br>${contractDate} | مدينة جدة</div></div>`;
  const footer = `<table class="footer-table"><tr><td>عقد تأجير مساحة - Area Seven</td><td></td></tr></table>`;
  const page = (body: string, first = false) => `<div class="page"><div class="page-inner">${first ? "" : header}<div class="content">${body}</div></div>${footer}</div>`;
  const section = (title: string) => `<div class="section-header">${title}</div>`;
  ...
  *{box-sizing:border-box;margin:0;padding:0}body{font-family:Cairo,Arial,Tahoma,sans-serif;background:#323639;color:#000;line-height:1.4;-webkit-print-color-adjust:exact}.page{width:210mm;height:297mm;padding:12mm 15mm;margin:15px auto;background:#fff;position:relative;box-shadow:0 0 10px rgba(0,0,0,.3);display:flex;flex-direction:column;justify-content:space-between;overflow:hidden;page-break-after:always;break-after:page}.page:last-of-type{page-break-after:avoid;break-after:auto}.page-inner{flex:1}.header-banner{border:1px solid #000;border-radius:4px;padding:6px 12px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center}.abaiti-logo{width:22mm;height:22mm;object-fit:contain;flex:0 0 auto}.header-title-box{font-size:8.5pt;font-weight:700;line-height:1.3}.header-logo-box{text-align:left;font-size:8pt;line-height:1.2}.header-logo-box strong{font-size:9.5pt;display:block}.content{display:flex;flex-direction:column;gap:4px}.main-title{text-align:center;font-size:15pt;font-weight:800;margin-bottom:2px}.sub-main-title{text-align:center;font-size:10.5pt;font-weight:700;margin-bottom:6px}.section-header{font-size:9.5pt;font-weight:800;margin-top:6px;margin-bottom:3px;border-right:3px solid #000;padding:2px 6px;background:#f8f8f8}p,li{font-size:8.8pt;text-align:justify;line-height:1.45}ol,ul{margin-right:18px}ol li{margin-bottom:2px}.data-table{width:100%;border-collapse:collapse;margin:6px 0;font-size:8.5pt}.data-table th,.data-table td{border:1px solid #000;padding:4px;text-align:center}.data-table th{background:#f0f0f0;font-weight:700}.signatures-container{display:flex;align-items:stretch;gap:12px;margin-top:15px}.signature-box{width:49%;height:120px;min-height:120px;border:none;padding:8px;border-radius:0;font-size:8.5pt;line-height:1.5;display:flex;flex-direction:column}.signature-box .sig-line{margin-top:auto;border-bottom:1px dotted #000;text-align:center}.footer-table{width:100%;border-collapse:collapse;border-top:1px solid #ccc;padding-top:4px;margin-top:8px;font-size:8pt;color:#555}.footer-table td{border:none!important;padding:0}@media print{body{background:none}.page{box-shadow:none;margin:0;width:210mm;height:297mm;padding:10mm 12mm}.controls{display:none!important}@page{size:A4 portrait;margin:0}}@media screen and (max-width:800px){.page{width:100%;height:auto;margin:0 0 12px;padding:20px}}
  </style><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800&display=swap" rel="stylesheet"></head><body><div class="controls"><button onclick="window.print()">طباعة العقد (3 صفحات)</button></div>
  ${page(`${`<div class="main-title">اتفاقية تأجير مساحة</div><div class="sub-main-title">بمعرض ملتقى أمن وسلامة الفعاليات بجدة</div>${section("أولاً: بيانات العقد")}<p><b>رقم العقد:</b> ${contractNumber}</p><p><b>تاريخ العقد:</b> ${contractDate}</p><p><b>مكان إبرام العقد:</b> مدينة جدة</p><p><b>وصف العقد:</b> عقد تأجير مساحة جناح (بوث) في معرض ملتقى أمن وسلامة الفعاليات الثقافية والفنية بجدة.</p>${section("ثانياً: أطراف العقد")}<p><b>الطرف الأول - المؤجر:</b> مؤسسة منطقة سبعة للإنتاج الإعلامي والمرئي والمسموع وتنظيم المعارض والمؤتمرات بموجب تصريح المزاولة رقم (160273)، والسجل التجاري رقم (4030618093)، وعنوانها المملكة العربية السعودية - مدينة جدة - حي الشرفية - طريق الملك فهد - 23218، ويمثلها السيد / سراج عمر خليل بصفته المدير العام.</p><p><b>الطرف الثاني - المستأجر:</b> اسم الجهة: ${company}، بموجب السجل التجاري رقم (${cr})، وعنوان مقرها في مدينة ${city}، ${address}، ويمثلها السيد/ ${representative}.</p><p>وقد تم إبرام هذه الاتفاقية بين الطرفين المذكورين أعلاه وفقاً للشروط والأحكام الواردة فيها.</p>${section("تمهيد")}<p>وحيث يرغب الطرف الأول في تسويق وتأجير مساحات للعارضين في المعرض المقام لمدة يومين بتاريخ 30-31 / 03 / 2026م في فندق هيلتون جدة - القاعة الكبرى، وحيث يرغب الطرف الثاني في المشاركة والتعريف بخدماته، فقد اتفق الطرفان على إبرام هذه الاتفاقية.</p>`}`, true)}
  ${page(`${section("البنود التفصيلية")}${section("البند الأول: التمهيد")}<p>يعد التمهيد الوارد أعلاه وما يرد على هذه الاتفاقية من مرفقات أو ملاحق يتفق عليها الطرفان ويتم التوقيع عليها لاحقاً جزءاً لا يتجزأ منها ومكملاً ومفسراً لأحكامها.</p>${section("البند الثاني: موضوع الاتفاقية")}<p>يشارك الطرف الثاني في المعرض المقام لمدة يومين في القاعة الكبرى بفندق هيلتون جدة من خلال الاشتراك في إحدى الباقات المتاحة، بهدف التواصل مع الزوار والمشاركين والتعريف بخدماته ومنتجاته وتسويقها.</p>${section("البند الثالث: التعريفات")}<ol><li><b>باقة الاشتراك:</b> الباقة التي يختارها الطرف الثاني وتحدد مزايا وخدمات المشاركة.</li><li><b>جناح العرض:</b> المساحة المخصصة للطرف الثاني ضمن مخطط القاعة.</li><li><b>العارضون:</b> الجهات والأفراد المشتركون في المعرض.</li><li><b>الظهور الرقمي:</b> ظهور اسم أو شعار الطرف الثاني عبر المنصات الرقمية.</li><li><b>منصة فورتيس:</b> منصة تجمع الجهات والمهتمين في مجال أمن وسلامة الفعاليات.</li></ol>${section("البند الرابع: مدة العقد")}<p>تكون مدة الاتفاقية يومين فقط وتنتهي بانتهاء فعاليات المعرض في نهاية اليوم الثاني، ما لم يتفق الطرفان كتابة على خلاف ذلك.</p>${section("البند الخامس: فئة الاشتراك والمساحة")}<p>فئة الاشتراك: ${category} | مساحة الجناح: ${size} | رقم الجناح: ${booth}</p><p>قيمة الاشتراك قبل الضريبة: ${money(amount)}، وتضاف ضريبة القيمة المضافة وفقاً للنسبة المطبقة نظاماً.</p>${section("البند السادس: تجهيزات جناح العرض")}<ol><li>يلتزم الطرف الأول بتوفير وتجهيز الجناح وفقاً للموقع والمساحة والفئة المحددة.</li><li>توفير لوحة تعريفية ومقابس الكهرباء الأساسية والتغطية الإعلامية المعتمدة.</li><li>يلتزم الطرف الثاني بالأنظمة والسلامة ويتحمل تكاليف التجهيزات الإضافية وأي أضرار ناتجة عنها.</li></ol>${section("البند السابع: المقابل المالي")}<table class="data-table"><tr><th>الفئة</th><th>المساحة</th><th>رقم الجناح</th><th>قبل الضريبة</th><th>الضريبة</th><th>الإجمالي</th></tr><tr><td>${category}</td><td>${size}</td><td>${booth}</td><td>${money(amount)}</td><td>${money(vat)}</td><td>${money(total)}</td></tr></table>`)}
  ${page(`${section("البند الثامن: حقوق والتزامات الطرف الأول")}<p>يلتزم الطرف الأول بتنفيذ التجهيزات والوفاء بمزايا الباقة والقيام بالحملات الإعلانية والتسويقية المعتمدة.</p>${section("البند التاسع: حقوق والتزامات الطرف الثاني")}<p>يلتزم الطرف الثاني بفترة التجهيز وإخلاء الجناح وتعليمات الدفاع المدني والأمن والسلامة وتحمل الأضرار والمخالفات الناتجة عنه.</p>${section("البند العاشر: النزاعات")}<p>تتم التسوية ودياً خلال خمسة أيام عمل، فإذا تعذر ذلك يحال الخلاف إلى الجهات المعنية في مدينة جدة.</p>${section("البند الحادي عشر: شمولية الاتفاقية")}<p>تعتبر الاتفاقية شاملة لجميع الشروط المتفق عليها ولا يعتد بأي اتفاق شفهي أو ملحق غير موقع.</p>${section("البند الثاني عشر: إنهاء الاتفاقية")}<p>لا يحق لأي من الطرفين إنهاء الاتفاقية بعد إبرامها وسداد قيمة الاشتراك إلا في حالة القوة القاهرة أو باتفاق كتابي.</p>${section("البند الثالث عشر: القوة القاهرة")}<p>إذا فشل أي طرف في تنفيذ التزاماته بسبب قوة قاهرة فلا يتحمل المسؤولية، وعلى الطرف المتضرر إبلاغ الآخر بالأدلة خلال سبعة أيام.</p>${section("البند الرابع عشر: أحكام عامة")}<p>اللغة العربية هي المعتمدة، ويلتزم الطرفان بالسرية وحماية الملكية الفكرية، ولا يجوز التنازل للغير دون موافقة كتابية.</p>${section("البند الخامس عشر: نسخ الاتفاقية")}<p>حررت الاتفاقية من نسختين أصليتين، يحصل كل طرف على نسخة للعمل بموجبها.</p><div class="signatures-container"><div class="signature-box"><b>الطرف الأول - المؤجر</b><br>مؤسسة منطقة سبعة للإنتاج الإعلامي والمرئي والمسموع وتنظيم المعارض والمؤتمرات<br>يمثلها: أ. سراج عمر خليل<div class="sig-line">التوقيع والختم</div></div><div class="signature-box"><b>الطرف الثاني - المستأجر</b><br>اسم الجهة: ${company}<br>يمثلها: أ. ${representative}<div class="sig-line">التوقيع والختم</div></div></div>`)}
...
export function RentalContractsPanel({locale}: {locale: string}) {
  ...
  const leads = useBackend<BackendRow[]>("/api/v1/data/leads", contractFormOpen);
  const contractSettings = useBackend<ContractSettings>("/api/v1/contract-settings?type=rental");
  ...
  useEffect(() => {
    const settings = contractSettings.data;
    if (!settings || !appliesToContract(settings, "rental") || editingContractId) return;
    const snapshot = JSON.stringify(settings);
    if (appliedContractSettingsRef.current === snapshot) return;
    appliedContractSettingsRef.current = snapshot;
    setEventName(settings.eventName || eventName);
    setEventDates(settings.eventDates || eventDates);
    setEventLocation(settings.eventLocation || eventLocation);
    setLessorName(settings.lessorName || lessorName);
    setFirstPartyCr(settings.firstPartyCr);
    setFirstPartyRepresentative(settings.firstPartyRepresentative);
    setCountry(settings.country || country);
    setParticipationCategory(settings.participationCategory || participationCategory);
    setBoothSize(settings.boothSize || boothSize);
    setCity(settings.city || city);
  }, [contractSettings.data, editingContractId]);
  ...
  const text = isArabic
    ? {
        ...
        updated: "تم تحديث العقد",
        failed: "تعذر حفظ العقد",
        validation: "أدخل اسم الشركة واسم الشخص المسؤول",
        noCustomerData: "اختر عميلاً مهتماً لتعبئة بيانات الشركة تلقائياً",
        ...
        updated: "Contract updated",
        failed: "Unable to save contract",
        validation: "Enter the company name and contact person",
        noCustomerData: "Select an interested customer to fill company details automatically",
  ...
  const boothGroups = useMemo(() => {
    ...
    for (const layoutBooth of NEW_BOOTH_LAYOUT) {
      const id = layoutBooth.id.toUpperCase();
      boothMap.set(id, {
        id,
        area: NEW_BOOTH_DIMENSION_OVERRIDES[id] ?? "",
      });
    }
  ...
  function selectBooth(nextBooth: string) {
    ...
    const booth =
      ...
      NEW_BOOTH_LAYOUT.find((item) => item.id.toUpperCase() === normalizedBooth);
    const catalogBooth = boothCatalogByNumber.get(normalizedBooth);
    setBoothNumber(normalizedBooth);
    const selectedBoothSize = String(
      catalogBooth?.booth_size ??
      (booth && "area" in booth ? booth.area : "") ??
      NEW_BOOTH_DIMENSION_OVERRIDES[normalizedBooth] ??
      "",
    ).trim();
    if (selectedBoothSize) setBoothSize(selectedBoothSize);
  }
  ...
  function resetContractForm() {
    setEditingContractId(null);
    appliedContractSettingsRef.current = "";
  ...
  function editContract(contract: BackendRow) {
    ...
    setLeaseStartDate(cleanDate(contract.lease_start_date) === "—" ? "" : cleanDate(contract.lease_start_date));
    setLeaseEndDate(cleanDate(contract.lease_end_date) === "—" ? "" : cleanDate(contract.lease_end_date));
    setUnitPrice(contract.unit_price == null ? "" : String(contract.unit_price));
    setQuantity(contract.quantity == null ? "" : String(contract.quantity));
    setContractDate(cleanDate(contract.contract_date) === "—" ? "" : cleanDate(contract.contract_date));
    setNotes(String(contract.notes ?? ""));
    setPaymentStatus(String(contract.payment_status ?? ""));
  ...
  function printContract(contract: BackendRow) {
    printAreaSevenRentalContractThreePage(contract);
  }
  ...
  return (
    <div className="quotes-page-grid">
      ...
      <article className="quote-card quote-form-card rental-contract-form-card">
        ...
        <div className="form-grid">
          <div className="rental-form-section-heading"><span>01</span><h4>{isArabic ? "المعلومات الأساسية والفعالية" : "Basic and event information"}</h4></div>
          ...
          <div className="rental-form-section-heading"><span>02</span><h4>{isArabic ? "أطراف العقد وبيانات التواصل" : "Contract parties and contact details"}</h4></div>
          <label className="quote-field quote-field-customer contract-lead-select-field">
            <span>{text.customer}</span>
            <DashboardSelect
              ariaLabel={text.customer}
          ...
          <label className="quote-field"><span>{text.email}</span><input onChange={(event) => setEmail(event.target.value)} type="email" value={email} /></label>
          <label className="quote-field"><span>{text.phone}</span><input inputMode="tel" onChange={(event) => setPhone(event.target.value)} value={phone} /></label>
          <div className="rental-form-section-heading"><span>03</span><h4>{isArabic ? "تفاصيل البوث والتأجير" : "Booth and rental details"}</h4></div>
          ...
          {boothPickerOpen ? (
            <div className="rental-booth-modal-backdrop" role="presentation">
              <section className="rental-booth-modal" aria-modal="true" role="dialog">
                ...
                <div className="admin-booth-zone-filter rental-booth-modal-zone-filter" role="listbox" aria-label={isArabic ? "فلترة الأقسام" : "Zone filter"}>
                  {FLOOR_MAP_ZONES.map((zone) => (
                    <button
                      ...
                      className={`${boothMapZone === zone.key ? "active" : ""} zone-${zone.key}`}
                      key={zone.key}
                ...
                <div className="admin-booth-map-legend rental-booth-modal-map-legend" aria-label={isArabic ? "دليل ألوان البوثات" : "Booth color legend"}>
                  <span>{isArabic ? "مختار" : "Selected"}<i className="selected" aria-hidden="true" /></span>
                  <span>{isArabic ? "متاح" : "Available"}<i className="available" aria-hidden="true" /></span>
                  <span>{isArabic ? "بانتظار الدفع" : "Pending payment"}<i className="pending-payment" aria-hidden="true" /></span>
                  <span>{isArabic ? "محجوز" : "Booked"}<i className="booked" aria-hidden="true" /></span>
          ...
          <label className="quote-field"><span>{text.rentalItem}</span><input onChange={(event) => setRentalItem(event.target.value)} value={rentalItem} /></label>
          ...
          <label className="quote-field rental-lease-start-field"><span>{text.leaseStartDate}</span><input onChange={(event) => setLeaseStartDate(event.target.value)} type="date" value={leaseStartDate} /></label>
          <label className="quote-field rental-contract-date-field"><span>{text.contractDate}</span><input onChange={(event) => setContractDate(event.target.value)} type="date" value={contractDate} /></label>
          <label className="quote-field rental-lease-end-field"><span>{text.leaseEndDate}</span><input onChange={(event) => setLeaseEndDate(event.target.value)} type="date" value={leaseEndDate} /></label>
          <div className="rental-form-section-heading"><span>04</span><h4>{isArabic ? "البيانات المالية والحالة" : "Financial details and status"}</h4></div>
          <label className="quote-field">
            ...
            <DashboardSelect
              ...
              placeholder={isArabic ? "اختر حالة السداد" : "Choose payment status"}
              value={paymentStatus}
            />
          </label>
          <label className="quote-field">
        ...
        <div className="rental-form-section-heading rental-form-section-heading-wide"><span>05</span><h4>{isArabic ? "العنوان والملاحظات" : "Address and notes"}</h4></div>
        <label className="quote-details-field"><span>{text.address}</span><textarea onChange={(event) => setAddress(event.target.value)} value={address} /></label>
        <label className="quote-details-field"><span>{text.notes}</span><textarea onChange={(event) => setNotes(event.target.value)} value={notes} /></label>
        <div className="quote-action-row">
