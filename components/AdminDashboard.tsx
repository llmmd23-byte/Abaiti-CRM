...
const newDGroups = [
  ...
  {start: 17, count: 4, left: 27, top: 68, columns: 2},
  {start: 21, count: 4, left: 48, top: 68, columns: 2},
  {start: 37, count: 4, left: 27, top: 101, columns: 2},
  {start: 41, count: 4, left: 48, top: 101, columns: 2},
...
export const NEW_BOOTH_LAYOUT = [
  ...
  {id: "B4", left: 30, top: 27, width: 18, height: 11},
  {id: "B5", left: 59, top: 27, width: 18, height: 11},
            ...
            <>
              ...
              >
                ...
                {visibleLayoutBooths.map((layoutBooth) => {
                  ...
                  return (
                    <button
                      className={`admin-booth-map-tile category-${boothCategory} booth-${layoutBooth.id.toLowerCase()} ${["A4", "A5", "A6", "C14"].includes(layoutBooth.id) ? "booth-gray" : ""} ${layoutBooth.id === "C14" ? "booth-c14" : ""} ${["C12", "C13"].includes(layoutBooth.id) ? "booth-gov" : ""} ${layoutBooth.id === "C15" ? "booth-c15" : ""} ${layoutBooth.id === "C16" ? "booth-c16" : ""} ${layoutBooth.width < 5 ? "is-narrow" : ""} ${isPolishedBooth ? "is-polished-booth" : ""} ${isFeatureArea ? "is-feature-area" : ""} ${hasSearch && !isSearchMatch ? "is-search-dimmed" : ""} ${hasSearch && isSearchMatch ? "is-search-match" : ""} ${activeZone !== "all" && activeZone !== layoutZone ? "is-zone-dimmed" : ""} ${activeZone === layoutZone ? "is-zone-focused" : ""} ${isPendingPayment ? "is-pending-payment" : ""} ${isBooked ? "is-booked" : ""} ${isInactive ? "is-inactive" : ""} ${isSelected ? "is-selected" : ""} ${isMissing ? "is-missing" : ""}`}
                      disabled={isMissing}
                      dir="ltr"
                      key={layoutBooth.id}
