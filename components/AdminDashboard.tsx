...
const EMPTY_CONTRACT_SETTINGS: ContractSettingsDraft = {
  ...
  registrationFee: "0",
  city: "Jeddah",
  eventName: "المعرض الدولي لصناع القهوة والشوكولاتة",
  eventDates: "8-10 أكتوبر 2026م (27-29 ربيع الآخر 1448هـ)",
  eventLocation: "فندق جدة هيلتون - القاعة الكبرى",
  lessorName: "شركة نطاق الأعمال لتنظيم المعارض والمؤتمرات",
  firstPartyCr: "",
  firstPartyRepresentative: "",
  country: "Saudi Arabia",
  rentalItem: "مساحة / جناح تأجيري",
  rentalLocation: "",
  participationCategory: "كلاسيك (Classic)",
  boothSize: "3x3 متر",
  rentalUnitPrice: "6500",
  rentalQuantity: "1",
...
function contractSettingsDraftFromValue(settings: Record<string, unknown> | undefined, scope: ContractSettingsDraft["contractTypeScope"]): ContractSettingsDraft {
  return {
    ...
    eventName: String(settings?.eventName ?? EMPTY_CONTRACT_SETTINGS.eventName),
    eventDates: String(settings?.eventDates ?? EMPTY_CONTRACT_SETTINGS.eventDates),
    eventLocation: String(settings?.eventLocation ?? EMPTY_CONTRACT_SETTINGS.eventLocation),
    lessorName: String(settings?.lessorName ?? EMPTY_CONTRACT_SETTINGS.lessorName),
    firstPartyCr: String(settings?.firstPartyCr ?? ""),
    firstPartyRepresentative: String(settings?.firstPartyRepresentative ?? ""),
    country: String(settings?.country ?? EMPTY_CONTRACT_SETTINGS.country),
    rentalItem: String(settings?.rentalItem ?? EMPTY_CONTRACT_SETTINGS.rentalItem),
    rentalLocation: String(settings?.rentalLocation ?? ""),
    participationCategory: String(settings?.participationCategory ?? EMPTY_CONTRACT_SETTINGS.participationCategory),
    boothSize: String(settings?.boothSize ?? EMPTY_CONTRACT_SETTINGS.boothSize),
    rentalUnitPrice: String(settings?.rentalUnitPrice ?? EMPTY_CONTRACT_SETTINGS.rentalUnitPrice),
    rentalQuantity: String(settings?.rentalQuantity ?? EMPTY_CONTRACT_SETTINGS.rentalQuantity),
    ...
    try {
      const response = await fetch("/api/v1/contract-settings", {
        ...
        body: JSON.stringify({
          ...
          lessorName: draft.lessorName,
          firstPartyCr: draft.firstPartyCr,
          firstPartyRepresentative: draft.firstPartyRepresentative,
          country: draft.country,
          rentalItem: draft.rentalItem,
          rentalLocation: draft.rentalLocation,
          participationCategory: draft.participationCategory,
          boothSize: draft.boothSize,
          rentalUnitPrice: Number(draft.rentalUnitPrice) || 0,
          rentalQuantity: Number(draft.rentalQuantity) || 0,
  ...
  return (
    <section className="admin-data-card contract-settings-panel" dir={isArabic ? "rtl" : "ltr"}>
      ...
      <div className="contract-settings-grid">
        ...
        <label><span>{isArabic ? "اسم الفعالية" : "Event name"}</span><input onChange={(event) => setDraft({...draft, eventName: event.target.value})} value={draft.eventName} /></label>
        <label><span>{isArabic ? "تاريخ العرض" : "Event dates"}</span><input onChange={(event) => setDraft({...draft, eventDates: event.target.value})} value={draft.eventDates} /></label>
        <label><span>{isArabic ? "مكان المعرض" : "Event location"}</span><input onChange={(event) => setDraft({...draft, eventLocation: event.target.value})} value={draft.eventLocation} /></label>
        <label><span>{isArabic ? "المؤجر / الطرف الأول" : "Lessor / first party"}</span><input onChange={(event) => setDraft({...draft, lessorName: event.target.value})} value={draft.lessorName} /></label>
        <label><span>{isArabic ? "السجل التجاري للطرف الأول" : "First party CR"}</span><input onChange={(event) => setDraft({...draft, firstPartyCr: event.target.value})} value={draft.firstPartyCr} /></label>
        <label><span>{isArabic ? "ممثل الطرف الأول" : "First party representative"}</span><input onChange={(event) => setDraft({...draft, firstPartyRepresentative: event.target.value})} value={draft.firstPartyRepresentative} /></label>
        <label><span>{isArabic ? "الدولة" : "Country"}</span><input onChange={(event) => setDraft({...draft, country: event.target.value})} value={draft.country} /></label>
        <label><span>{isArabic ? "وصف العين المؤجرة" : "Rental item"}</span><input onChange={(event) => setDraft({...draft, rentalItem: event.target.value})} value={draft.rentalItem} /></label>
        <label><span>{isArabic ? "موقع التأجير" : "Rental location"}</span><input onChange={(event) => setDraft({...draft, rentalLocation: event.target.value})} value={draft.rentalLocation} /></label>
        <label><span>{isArabic ? "فئة المشاركة" : "Participation category"}</span><input onChange={(event) => setDraft({...draft, participationCategory: event.target.value})} value={draft.participationCategory} /></label>
        <label><span>{isArabic ? "مساحة البوث" : "Booth size"}</span><input onChange={(event) => setDraft({...draft, boothSize: event.target.value})} value={draft.boothSize} /></label>
        <label><span>{isArabic ? "قيمة الإيجار مع الضريبة" : "Rental value with VAT"}</span><input min="0" onChange={(event) => setDraft({...draft, rentalUnitPrice: event.target.value})} type="number" value={draft.rentalUnitPrice} /></label>
        <label><span>{isArabic ? "الكمية / المدة" : "Quantity / period"}</span><input min="0" onChange={(event) => setDraft({...draft, rentalQuantity: event.target.value})} type="number" value={draft.rentalQuantity} /></label>
      </div>
...
const newDGroups = [
  {ids: [35, 36, 37], left: 0.5, top: 6},
  {ids: [38, 39, 40], left: 17.5, top: 6},
  {ids: [41, 42, 43], left: 34.5, top: 6},
  {ids: [44, 45, 46], left: 51.5, top: 6},
  {ids: [47, 48, 49], left: 68.5, top: 6},
  {ids: [50, 51], left: 85.5, top: 6},
  {ids: [18, 19, 20, 1, 2, 3], left: 0.5, top: 19},
  {ids: [21, 22, 23, 4, 5, 6], left: 17.5, top: 19},
  {ids: [24, 25, 26, 7, 8, 9], left: 34.5, top: 19},
  {ids: [27, 28, 29, 10, 11, 12], left: 51.5, top: 19},
  {ids: [30, 31, 32, 13, 14, 15], left: 68.5, top: 19},
  {ids: [33, 34, 16, 17], left: 85.5, top: 19},
] as const;
...
export const NEW_BOOTH_LAYOUT = [
  ...
  ...([
    ["M10", 1, 37.4], ["M11", 8.8, 37.4], ["M1", 1, 45.4], ["M2", 8.8, 45.4],
    ["M12", 20.2, 37.4], ["M13", 28, 37.4], ["M3", 20.2, 45.4], ["M4", 28, 45.4],
    ["M14", 39.4, 37.4], ["M15", 47.2, 37.4], ["M5", 39.4, 45.4], ["M6", 47.2, 45.4],
    ["M16", 58.6, 37.4], ["M17", 66.4, 37.4], ["M7", 58.6, 45.4], ["M8", 66.4, 45.4],
    ["M18", 77.8, 37.4], ["M9", 77.8, 45.4],
  ] as const).map(([id, left, top]) => ({id, left, top, width: 7.8, height: 8})),
...
export const NEW_BOOTH_GROUPS = [
  ...[
    {left: 0.1, top: 5.6, width: 13.4, height: 6.6, kind: "d-top"},
    {left: 17.1, top: 5.6, width: 13.4, height: 6.6, kind: "d-top"},
    {left: 34.1, top: 5.6, width: 13.4, height: 6.6, kind: "d-top"},
    {left: 51.1, top: 5.6, width: 13.4, height: 6.6, kind: "d-top"},
    {left: 68.1, top: 5.6, width: 13.4, height: 6.6, kind: "d-top"},
    {left: 85.1, top: 5.6, width: 9.2, height: 6.6, kind: "d-top"},
    {left: 0.1, top: 18.6, width: 13.4, height: 12.4, kind: "d-middle"},
    {left: 17.1, top: 18.6, width: 13.4, height: 12.4, kind: "d-middle"},
    {left: 34.1, top: 18.6, width: 13.4, height: 12.4, kind: "d-middle"},
    {left: 51.1, top: 18.6, width: 13.4, height: 12.4, kind: "d-middle"},
    {left: 68.1, top: 18.6, width: 13.4, height: 12.4, kind: "d-middle"},
    {left: 85.1, top: 18.6, width: 9.2, height: 12.4, kind: "d-middle"},
  ] as const,
  ...[
    {left: 1, top: 37.4, width: 15.6, height: 17},
    {left: 20.2, top: 37.4, width: 15.6, height: 17},
    {left: 39.4, top: 37.4, width: 15.6, height: 17},
    {left: 58.6, top: 37.4, width: 15.6, height: 17},
    {left: 77.8, top: 37.4, width: 7.8, height: 17},
...
<div className={`admin-booths-workspace ${selectedBooth ? "has-selected-booth" : ""}`}>
        <div className="admin-booths-layout" aria-busy={isLoading}>
          {isLoading ? (
            ...
            <>
              {!selectedBooth ? (
                <div className="admin-booth-stats admin-booth-map-stats">
                  <span className="total"><i className="status-dot total-dot" aria-hidden="true" /><strong>{booths.length.toLocaleString(NUMBER_LOCALE)}</strong> {isArabic ? "بوث" : "booths"}</span>
                  <span className="available"><i className="status-dot available-dot" aria-hidden="true" /><strong>{totalAvailable.toLocaleString(NUMBER_LOCALE)}</strong> {isArabic ? "متاح" : "available"}</span>
                  <span className="booked"><i className="status-dot booked-dot" aria-hidden="true" /><strong>{totalBooked.toLocaleString(NUMBER_LOCALE)}</strong> {isArabic ? "محجوز" : "booked"}</span>
                  <span className="pending-payment"><i className="status-dot pending-dot" aria-hidden="true" /><strong>{totalPendingPayment.toLocaleString(NUMBER_LOCALE)}</strong> {isArabic ? "بانتظار الدفع" : "pending"}</span>
                </div>
              ) : null}
              <div
